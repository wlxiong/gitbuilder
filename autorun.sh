...
run_script() {
    ...
    # run build
    if [ -f "$script" ]; then
        timeout $timeout $script $ref | tee out/log | tee build_log
    else
...
while [ "$(ls -A out/pending)" ]; do
    for file in out/pending/*; do
        ...
        [ "$action" = "build" ]  && run_script build/ci/run-build.sh $ref 180
        [ "$action" = "review" ] && run_script build/ci/run-review.sh $ref 600
        rm -f $file
    done
done
